...
#pragma warning restore 0618
        ...
        /// <summary>
        /// Explicitly binds Crest's live displacement texture and cascade data to a
        /// custom material. URP full-screen renderer features can otherwise retain
        /// material-local defaults instead of consuming Crest's global bindings.
        /// </summary>
        public void BindCustomMaterialData(Material material)
        {
            if (material == null || _lodDataAnimWaves == null || _bufCascadeDataTgt == null) return;

            material.SetTexture(LodDataMgrAnimWaves.ParamIdSampler(), _lodDataAnimWaves.DataTexture);
            material.SetBuffer(sp_cascadeData, _bufCascadeDataTgt);
            material.SetBuffer(sp_CrestCascadeDataSource, _bufCascadeDataSrc);
            material.SetFloat(sp_sliceCount, CurrentLodCount);
            material.SetFloat(sp_crestTime, CurrentTime);
            material.SetVector(sp_oceanCenterPosWorld, Root != null ? Root.position : transform.position);
            material.SetFloat(sp_meshScaleLerp, ScaleCouldIncrease ? ViewerAltitudeLevelAlpha : 0f);
        }
