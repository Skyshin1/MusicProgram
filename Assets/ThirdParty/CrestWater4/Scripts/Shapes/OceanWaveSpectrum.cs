...
#pragma warning restore 414
        ...
        /// <summary>
        /// Stable runtime hash of every setting that changes the generated FFT spectrum.
        /// This lets authoring tools invalidate Crest's cached FFT generator without
        /// exposing the spectrum's serialized per-octave storage.
        /// </summary>
        public int ComputeSettingsHash()
        {
            unchecked
            {
                var hash = 17;
                hash = hash * 31 + _multiplier.GetHashCode();
                hash = hash * 31 + _chop.GetHashCode();
                hash = hash * 31 + _waveDirectionVariance.GetHashCode();
                hash = hash * 31 + _gravityScale.GetHashCode();
                hash = hash * 31 + _smallWavelengthMultiplier.GetHashCode();
                for (var index = 0; index < NUM_OCTAVES; index++)
                {
                    hash = hash * 31 + _powerLog[index].GetHashCode();
                    hash = hash * 31 + _powerDisabled[index].GetHashCode();
                    hash = hash * 31 + _chopScales[index].GetHashCode();
                    hash = hash * 31 + _gravityScales[index].GetHashCode();
                }
                return hash;
            }
        }
