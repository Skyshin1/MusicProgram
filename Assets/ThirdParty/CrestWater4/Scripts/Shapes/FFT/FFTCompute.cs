...
namespace Crest
{
    ...
    public class FFTCompute
    {
        ...
        /// <summary>
        /// Rebuild initial spectrum data for generators using this spectrum.
        /// Supports runtime sea-state authoring without allocating a new FFT
        /// generator for every parameter adjustment.
        /// </summary>
        public static void InvalidateSpectrum(OceanWaveSpectrum spectrum)
        {
            if (spectrum == null || _generators == null) return;
            foreach (var generator in _generators.Values)
            {
                if (generator._spectrum == spectrum)
                {
                    generator._spectrumInitialised = false;
                }
            }
