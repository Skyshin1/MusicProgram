...
- **Room_BlackBox_Analysis（上层解析室）**：黑匣子插座、水样接收座。
...
沿灯标上浮，在平台外橙色 `Board Platform` 面板附近，用空手 Trigger 登乘。登乘位置有碰撞检查和淡入淡出。

把黑匣子带到原平台上层解析室，**实际放入左侧插座**。仅靠近、查看或点击附近区域不会完成解析。插入后等待录音结束，出现证据分析面板。
