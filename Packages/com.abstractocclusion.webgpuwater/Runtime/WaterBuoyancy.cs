...
namespace AbstractOcclusion.WebGpuWater
{
    ...
    public partial class WaterBuoyancy : MonoBehaviour
    {
        ...
        /// <summary>
        /// Runtime multiplier for all forces generated by this component.
        /// Gameplay grab bridges use it to soften or suspend buoyancy while an
        /// XR interactor owns the Rigidbody without changing authored values.
        /// </summary>
        float _externalForceScale = 1f;
        public float ExternalForceScale
        {
            get => _externalForceScale;
            set => _externalForceScale = Mathf.Clamp01(value);
        }
        ...
        void ApplyPointForces(Vector3 world, WaterSample sample, Vector3 up, float gravity, float weight)
        {
            ...
            _rb.AddForceAtPosition(lift * ExternalForceScale, world, ForceMode.Acceleration);

            // Per-point drag so the object settles (damps translation and rotation).
            _rb.AddForceAtPosition(
                -SurfaceDrag(world, sample) * (weight * ExternalForceScale),
                world,
            ...
            _rb.AddForceAtPosition(
                drift * (waveDriftStrength * weight * ExternalForceScale),
                world,
                ForceMode.Acceleration);
        }
        ...
        {
            _rb.AddTorque(
                -_rb.angularVelocity * (waterAngularDamping * averageFraction * ExternalForceScale),
                ForceMode.Acceleration);
            float verticalSpeed = Vector3.Dot(_rb.linearVelocity, up);
            _rb.AddForce(
                up * (-verticalSpeed * verticalSettleDamping * averageFraction * ExternalForceScale),
                ForceMode.Acceleration);
        }
